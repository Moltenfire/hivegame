...
pub(super) struct AnalysisState {
    ...
    /// `None` is the ordinary empty-board root. See [`super::document::load_root`].
...
/// A canonical match keeps the node's own move notation, which renames interchangeable pieces;
/// continuations are recorded against those names, so the board is replayed to adopt them.
/// `allow_shared` off leaves only Exact reuse, which needs no replay and so cannot fail.
fn plan_append(
    ...
    let mut reused_canonical_child = false;
    for ((piece, position), hash) in moves {
        ...
        let id = match arena.matching_child(selected, &value, *hash) {
            ...
            Some(ChildMatch::Canonical(id)) if allow_shared => {
                reused_canonical_child = true;
                id
            }
            Some(ChildMatch::Canonical(_)) | None => break,
        };
        selected = id;
        depth = turn;
        matched_count += 1;
