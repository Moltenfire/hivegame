...
use hive_lib::{Color, GameResult, GameStatus, GameType, State};
use leptos::prelude::*;
...
/// it twice and once.
const TRANSPOSITION_PREFIX: &str = r"wL ;bL wL-;wM -wL;bM bL-;wA1 wM\;bQ \bM;wQ /wM;bA1 bL\;wA2 \wL;bP bQ/;wA2 bA1\;bA2 bP-;wP \wQ;bA3 \bP;wA1 -bA3;bG1 \bA2;wA3 wM\;bA2 wA3\;wM bM-;bS1 bG1\;wB1 wM\;bA2 wB1\;wB2 \wA3;bG1 bP\;wB2 wL;bS1 bA2/;wG1 \wA3;bA2 wA3\;wG1 \bS1;bB1 /bA2;wB1 wM;bM wB1;wB2 bL;bB1 wQ\;wG2 \wA3;bB1 wQ;wP -bB1;bB1 wP;wB2 bQ;bS2 bA2\;wG2 bS2\;bG2 /bB1;wS1 /wA1;bG2 \wQ;wS1 \bA3;bS1 wA2-;wS2 \wA1;bS1 wG1\;wA1 bS1\;bG2 wQ\;wS2 \bL;bG2 \wQ;wB2 bP;bG3 /bA2;wA1 /bG3;bS1 wA2-";
const TRANSPOSITION_CYCLE: &str = r"wA1 -bG3;bG2 wQ\;wA1 /bG3;bG2 \wQ";
const TRANSPOSITION_M: &str = r"wB2 bA3;bM bG1";

fn plies(text: &str) -> Vec<(String, String)> {
    text.split_terminator(';')
        .map(|mov| {
            let (piece, position) = mov.split_once(' ').expect("piece and position");
            (piece.to_string(), position.to_string())
        })
        .collect()
}

#[test]
fn a_transposed_line_counts_only_its_own_visits() {
    let owner = Owner::new();
    owner.with(|| {
        let prefix = plies(TRANSPOSITION_PREFIX);
        let cycle = plies(TRANSPOSITION_CYCLE);
        let m = plies(TRANSPOSITION_M);

        let mut line_a = prefix.clone();
        line_a.extend(m.iter().cloned());
        line_a.extend(cycle.iter().cloned());
        let loaded = LoadedAnalysis::from_moves(GameType::MLP, &line_a, &[], prefix.len()).unwrap();
        let game_state = GameStateStore::new();
        game_state.reset_with_state(loaded.playable);
        let store = AnalysisStore::new(loaded.state);

        let play = |moves: &[(String, String)]| {
            for (piece, position) in moves {
                let hash = game_state
                    ...
                    .unwrap();
                store.append_moves(vec![((piece.clone(), position.clone()), hash)], game_state);
            }
        };

        // Line B off the same ancestor, arriving at the position line A already holds.
        play(&cycle);
        play(&m);
        play(&cycle);
        game_state.state().with_untracked(|state| {
            let position = *state.hashes.last().expect("a position was reached");
            assert_eq!(
                state.hashes_count.get(&position),
                Some(&2),
                "line B has been here twice; line A's extra visit is not its own"
            );
            assert_eq!(state.game_status, GameStatus::InProgress);
        });

        // A third visit is a threefold on anyone's count.
        play(&cycle);
        game_state.state().with_untracked(|state| {
            let position = *state.hashes.last().expect("a position was reached");
            assert_eq!(state.hashes_count.get(&position), Some(&3));
            assert_eq!(state.game_status, GameStatus::Finished(GameResult::Draw));
        });
    });
}

/// Sharing the node would hand this line the other one's hash history, and every repetition
/// count is read off that history - a threefold here is not a threefold there.
#[test]
fn transposed_move_order_records_its_own_branch() {
    let owner = Owner::new();
    owner.with(|| {
        ...
        // All three divergent plies became real nodes, ply 5 included.
        assert_eq!(
            store.0.selected_path().get_untracked(),
            vec![
                NodeId(0),
        ...
        store.0.arena().with_untracked(|arena| {
            assert_eq!(arena.nodes.len(), node_count + 3);
            // The transposition is still recognised - the two ply-5 nodes are the same
            // position - it just no longer collapses the two lineages onto one node.
            assert_eq!(
                arena.node(convergence_target).unwrap().hash,
                arena.node(NodeId(8)).unwrap().hash,
            );
        });
        game_state.state().with_untracked(|state| {
            assert_eq!(state.hashes, played_hashes);
        });
    });
}
...
/// Copy HOP on a blank analysis emits the empty board, and loading that back has to land on the
/// same tree. A root hash sends the opening explorer to `game_hashes`, which holds no pre-move
/// rows, so one empty board would list the openings and the other nothing. `base,,w` is
/// reachable too: every blank analysis is MLP, but a loaded Base game rewound to its start is an
/// empty Base board, and its game type has to survive the round trip.
#[test]
fn an_empty_board_hop_loads_as_a_blank_analysis() {
    for (hop, game_type) in [(",w", GameType::MLP), ("base,,w", GameType::Base)] {
        let blank = super::store::AnalysisState::blank(game_type);
        let loaded = LoadedAnalysis::from_hop(hop).expect("valid HOP");
        let root_hash_of = |state: &super::store::AnalysisState| {
            state.arena.node(state.arena.root).expect("a root").hash
        };

        assert_eq!(loaded.state.game_type, game_type, "{hop}");
        assert_eq!(root_hash_of(&loaded.state), root_hash_of(&blank), "{hop}");
        assert_eq!(loaded.state.start_hop, None, "{hop}");
        assert!(
            !loaded
                .state
                .checkpoints
                .contains_key(&loaded.state.arena.root),
            "{hop}: a blank root carries no checkpoint"
        );
        assert_eq!(loaded.playable.board.played, 0, "{hop}");
    }
}

/// The root state and the root hash come from the same validated HOP. A position that parses
/// but no game reaches used to load as an empty board still carrying the real position's hash,
/// so the tree's state and its hash described different boards.
#[test]
fn a_start_position_the_root_cannot_hold_is_rejected() {
    for start_hop in ["base,AAAB,w", "base,not a hop,w"] {
        ...
        let document = AnalysisDocument {
            ...
            start_hop: Some(start_hop.to_string()),
        };
        let Err(error) = LoadedAnalysis::from_json(&serde_json::to_string(&document).unwrap())
        else {
            panic!("{start_hop} must not load");
        };
        assert!(
            matches!(error, LoadError::Invalid(_)),
            "{start_hop}: {error:?}"
        );
    }
}
...
/// `engine/test_pgns/hash/short_pass.pgn` reaches this position at ply 9 and answers it with a
/// pass. Analysis only ever submits piece-and-target moves, so a root whose side to move is shut
/// out loads fine and then cannot be continued at all.
#[test]
fn a_shutout_start_position_can_still_be_continued() {
    let owner = Owner::new();
    owner.with(|| {
        let loaded = LoadedAnalysis::from_hop("L!+lQq4=B,b").expect("valid HOP");
        let color = loaded.playable.turn_color;
        let (piece, target) = loaded
            .playable
            .board
            .moves(color)
            .into_iter()
            .find_map(|((piece, _), targets)| targets.first().map(|target| (piece, *target)))
            .unwrap_or_else(|| {
                panic!("{color} is to move on the imported root with nothing to play")
            });
        ...
        let appended = game_state
            .state()
            .try_update(|state| {
                let previous = state.history.moves.len();
                state.play_turn_from_position(piece, target).unwrap();
                state.history.moves[previous..]
                    .iter()
                    .cloned()
                    .zip(state.hashes[previous..].iter().copied())
                    .collect::<Vec<_>>()
            })
            .unwrap();
        store.append_moves(appended, game_state);
        assert_ne!(
            store.selected_node_id(),
            NodeId::ROOT,
            "the continuation must land in the tree"
        );
    });
}

/// The forced pass is a tree node, not a rewrite of the root: the pasted position is what the
/// user asked to analyse, and `start_hop` plus the root hash anchor the opening explorer to it.
#[test]
fn a_shutout_start_position_keeps_the_pasted_root() {
    let owner = Owner::new();
    owner.with(|| {
        let hop = "L!+lQq4=B,b";
        let loaded = LoadedAnalysis::from_hop(hop).expect("valid HOP");
        assert_eq!(loaded.state.start_hop.as_deref(), Some(hop));
        assert_eq!(
            loaded
                .state
                .arena
                ...
                .and_then(|node| node.hash),
            Some(hive_lib::hop::to_hash(hop).expect("hashable") as u64),
            "the root still hashes as the pasted position"
        );
        assert_eq!(
            loaded.playable.turn_color,
            Color::White,
            "the pass hands the move to the side that has one"
        );
        ...
        assert_eq!(
            ...
            "the analysis opens on the pass, so the board is ready to move"
        );

        let game_state = GameStateStore::new();
        game_state.reset_with_state(loaded.playable.clone());
        let store = AnalysisStore::new(loaded.state);
        let json = store.to_json().unwrap();
        let reloaded = LoadedAnalysis::from_json(&json)
            .unwrap_or_else(|error| panic!("a saved shutout-rooted analysis must reload: {error}"));
        assert_eq!(
            state_hop(&reloaded.playable),
            state_hop(&loaded.playable),
            "reloading must replay the pass, not stop at the root"
        );
    });
}
